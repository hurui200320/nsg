 ...
 * The timestamp is always freshly generated.
 */
class NikonPairingEngine(
    ...
    /**
     * Build stage 1. If [savedDevice] and [savedNonce] is provided, its device/nonce are reused.
     */
    fun createStage1(savedDevice: Long?, savedNonce: Long?): PairingMessage {
        val timestamp = random.nextLong()
        val device = savedDevice ?: generateDeviceId()
        val nonce = savedNonce ?: generateNonce()
    ...
    fun verifyStage2AndBuildStage3(
        ...
        val salt = findSalt(stage1, stage2) ?: error("Failed to find salt for $stage1 - $stage2")
