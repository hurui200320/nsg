# Nikon Z Bluetooth Smart-Mode GPS/Time Protocol
...
The goal is to connect to a Nikon Z camera over BLE and periodically send:

* **GPS location** (latitude, longitude, altitude, satellites, fix time) on the `GEO` characteristic.
* **Time** embedded inside the GPS payload.  A separate `TIME` characteristic is documented below but is not known to be required for GPS-only operation.

The camera must be placed in **“Connect to smart device” → pairing mode**.
...
All UUIDs share the base:

```
xxxxxxxx-3dd4-4255-8d62-6dc7b9bd5561
```

| Handle | Full UUID                                  | Role in smart mode                                              |
|--------|--------------------------------------------|-----------------------------------------------------------------|
| `0xDE00` | `0000de00-3dd4-4255-8d62-6dc7b9bd5561`   | Primary service                                                 |
| `0x2000` | `00002000-3dd4-4255-8d62-6dc7b9bd5561`   | **PAIR** — write pairing messages, receive indications            |
| `0x2008` | `00002008-3dd4-4255-8d62-6dc7b9bd5561`   | **NOT1** — success notification (`01 00`)                       |
| `0x2002` | `00002002-3dd4-4255-8d62-6dc7b9bd5561`   | **ID** — write the controller’s friendly name                   |
| `0x2006` | `00002006-3dd4-4255-8d62-6dc7b9bd5561`   | **TIME** — time sync (defined but not observed to be required)  |
| `0x2007` | `00002007-3dd4-4255-8d62-6dc7b9bd5561`   | **GEO** — GPS location payload (41 bytes)                         |
| `0x200A` | `0000200a-3dd4-4255-8d62-6dc7b9bd5561`   | NOT2 — declared but unused in the smart mode flow               |

Other characteristics in the same service exist (e.g., `0x2087` for remote-mode pairing), but they are not needed for GPS/time smart-mode operation.

---

## 3. Device Discovery

### 3.1 Initial pairing advertisements

When the camera is in **“Connect to smart device → Pairing (Bluetooth) → Start pairing”**, it advertises:

* The **primary service UUID** `0000de00-3dd4-4255-8d62-6dc7b9bd5561`.
* **No manufacturer data**.

Filter: service UUID matches and the advertisement does **not** contain manufacturer data.

### 3.2 Reconnecting a saved camera

After a successful pairing, the camera advertises **manufacturer data** when it is in range and the same device is expected:

| Byte | Field        | Value / Notes                              |
|------|--------------|--------------------------------------------|
| 0–1  | `companyID`  | `0x0399` (Nikon), little-endian            |
| 2–5  | `device`     | The saved 4-byte `device` id, little-endian |
| 6    | `zero`       | `0x00`                                     |
...
The controller should advertise a readable name.  The reference implementation uses `furble-xxxxx`, where `xxxxx` is a short hex identifier.  Any short ASCII string is likely fine; it is written to the `ID` characteristic at the end of pairing.

---

## 4. Smart-Mode Pairing Handshake

### 4.1 Pairing message format

Every pairing message is a **17-byte packed structure** sent on the `PAIR` characteristic.

| Byte(s) | Size | Field          | Description                                          |
|---------|------|----------------|------------------------------------------------------|
| 0       | 1    | `stage`        | Stage counter: `0x01`, `0x02`, `0x03`, `0x04`, `0x05` |
| 1–8     | 8    | `timestamp`    | `uint64_t`, little-endian on the wire                |
| 9–12    | 4    | `id.device`    | `uint32_t`, little-endian                            |
| 13–16   | 4    | `id.nonce`     | `uint32_t`, little-endian                            |

In stages 4 and 5 the last 8 bytes are interpreted as an ASCII serial number (`serial[8]`) instead of `id`.

### 4.2 Stage flow

```text
App  ──stage 1 (write)──────> Camera
App  <──stage 2 (indicate)──── Camera
App  ──stage 3 (write)──────> Camera
App  <──stage 4 (indicate)──── Camera
App  ──stage 5 (write)──────> Camera
App  <──success (notify)────── Camera   on NOT1: 01 00
App  ──controller name (write)──> Camera   on ID characteristic
```

### 4.3 Stage message contents

| Stage | Direction | `timestamp` field                              | Last 8 bytes                               |
|-------|-----------|------------------------------------------------|--------------------------------------------|
| 1     | App → Cam | Random `uint64_t` generated by the controller  | Random `device`/`nonce`                    |
| 2     | Cam → App | Camera-generated timestamp                     | Camera-generated challenge id              |
| 3     | App → Cam | Same as stage 1                                | Blowfish response derived from stage 2     |
| 4     | Cam → App | Same as stage 2                                | ASCII camera serial number                 |
| 5     | App → Cam | `0x0000000000000000`                           | `0x0000000000000000`                       |

For stage 1, generate a random 64-bit timestamp and a random 32-bit `nonce`.  The `device` id should be a random 32-bit value whose **first byte on the wire is `0x01`**; in practice this means the least-significant byte of the `uint32_t` should be `0x01`.  The remaining three bytes are random.
...
## 5. Blowfish Challenge/Response

Stages 2 and 3 use a custom challenge/response built on top of **standard Blowfish** with a fixed key and a fixed table of 8 salts.
...
**8 salt values** (each salt is two 32-bit words):
...
### 5.2 Standard Blowfish initial P-array

Use the standard Blowfish key schedule with this initial P-array (18 × 32-bit words):
...
The S-boxes are the standard Blowfish S-boxes (pi-derived).  Any standard Blowfish implementation initialized with the key above will produce the correct P-array and S-boxes after key scheduling.

### 5.3 Hash function

The protocol uses a custom CBC-MAC-like hash on top of Blowfish encipher:

```c
hash(src, len)  // len is even, in 32-bit words
{
    ...
    for (i = 0; i < len; i += 2) {
        inL = src[i]   ^ left;
        inR = src[i+1] ^ right;
        blowfish_encipher(&inL, &inR);   // standard Blowfish with the keyed P/S
        left  = inL;
        right = inR;
    }
...
### 5.4 Endianness convention

All values on the BLE wire are **little-endian**.  The hash function itself operates on **big-endian 32-bit words** of the timestamp halves.  In other words, before hashing a 32-bit timestamp half, convert it from little-endian (wire) to big-endian/host representation.

A helper way to think about it:

```c
// received on the wire
uint32_t cam_ts_lo  = le32_to_cpu(stage2.timestamp bytes 1..4);
uint32_t cam_ts_hi  = le32_to_cpu(stage2.timestamp bytes 5..8);
uint32_t our_ts_lo  = le32_to_cpu(stage1.timestamp bytes 1..4);
uint32_t our_ts_hi  = le32_to_cpu(stage1.timestamp bytes 5..8);

// values fed into the hash are the big-endian equivalents
uint32_t h_cam_ts_hi = htobe32(cam_ts_hi);
uint32_t h_cam_ts_lo = htobe32(cam_ts_lo);
uint32_t h_our_ts_hi = htobe32(our_ts_hi);
uint32_t h_our_ts_lo = htobe32(our_ts_lo);
```

### 5.5 Verifying stage 2 and finding the salt

For each salt index `i` from 0 to 7:

```c
src[6] = {
    SALT[i][0],
    SALT[i][1],
    h_cam_ts_hi, h_cam_ts_lo,   // big-endian timestamp halves from stage 2
    h_our_ts_hi, h_our_ts_lo    // big-endian timestamp halves from stage 1
};

(h0, h1) = hash(src, 6);

if (h0 == htobe32(stage2.id.device) && h1 == htobe32(stage2.id.nonce)) {
    matched_salt = i;
    break;
}
```

If none of the 8 salts match, the handshake should abort.

### 5.6 Computing the stage 3 response

Using the matched salt and the **timestamps in the opposite order**:

```c
src[6] = {
    SALT[matched_salt][0],
    SALT[matched_salt][1],
    h_our_ts_hi, h_our_ts_lo,   // our stage-1 timestamp
    h_cam_ts_hi, h_cam_ts_lo    // camera's stage-2 timestamp
};

(h0, h1) = hash(src, 6);

stage3.id.device = htole32(h0);   // send little-endian on the wire
stage3.id.nonce  = htole32(h1);
stage3.timestamp = stage1.timestamp;   // same 64-bit timestamp as stage 1
stage3.stage     = 0x03;
```
...
|--------|------|-------------------|------------------------------------|
| 0      | 2    | year              | `uint16_t`, little-endian          |
| 2      | 1    | month             | 1–12                               |
| 3      | 1    | day               | 1–31                               |
| 4      | 1    | hour              | 0–23                               |
| 5      | 1    | minute            | 0–59                               |
| 6      | 1    | second            | 0–59                               |
| 7      | 1    | dst_offset        | Daylight-saving offset (semantics unknown) |
...
## 7. GPS Location Payload (`GEO` characteristic)

The `GEO` payload is **41 bytes**.  It is written to the `GEO` characteristic (`0x2007`) whenever a fresh GPS fix is available.

| Offset | Size | Field                  | Value / Encoding                                      |
|--------|------|------------------------|-------------------------------------------------------|
| 0      | 2    | `header`               | `0x007F` little-endian (flags for lat/lon/sat/alt/pos/gps/map) |
| 2      | 1    | `latitude_direction`   | `'N'` (0x4E) or `'S'` (0x53)                         |
| 3      | 1    | `latitude_degrees`     | integer degrees (0–90)                                |
| 4      | 1    | `latitude_minutes`     | integer minutes (0–59)                                |
| 5      | 1    | `latitude_submin1`     | hundredths of a minute (0–99)                         |
| 6      | 1    | `latitude_submin2`     | hundredths of the remainder (0–99)                    |
| 7      | 1    | `longitude_direction`  | `'E'` (0x45) or `'W'` (0x57)                         |
| 8      | 1    | `longitude_degrees`    | integer degrees (0–180)                               |
| 9      | 1    | `longitude_minutes`    | integer minutes (0–59)                                |
| 10     | 1    | `longitude_submin1`    | hundredths of a minute (0–99)                         |
| 11     | 1    | `longitude_submin2`    | hundredths of the remainder (0–99)                    |
| 12     | 1    | `satellites`           | number of satellites used for the fix                   |
| 13     | 1    | `altitude_ref`         | `'P'` (0x50) positive, `'M'` (0x4D) negative altitude  |
| 14     | 2    | `altitude`             | absolute altitude in meters, `uint16_t` little-endian   |
| 16     | 7    | `time`                 | embedded 7-byte time structure (see below)            |
| 23     | 1    | `subseconds`           | centiseconds (0–99)                                   |
| 24     | 1    | `valid`                | `0x01` = valid fix                                    |
| 25     | 6    | `standard`             | ASCII `"WGS-84"`                                      |
| 31     | 10   | `pad`                  | all `0x00`                                            |

Total: **41 bytes**.

### 7.1 Embedded time structure (7 bytes)
...
Nikon encodes latitude/longitude as **degrees + minutes + two sub-minute bytes** rather than decimal degrees or degrees/minutes/seconds.  The fractional part of the minute is represented as a 4-digit number: `submin1` is the hundredths of a minute, and `submin2` is the hundredths of the remainder.

```c
void decimal_to_nikon(double decimal,
                      uint8_t &direction,
                      uint8_t &degrees,
                      uint8_t &minutes,
                      uint8_t &submin1,
                      uint8_t &submin2)
{
    direction = (decimal < 0.0) ? 'S' : 'N';   // or 'W'/'E' for longitude

    double absv = fabs(decimal);
    double integral;

    std::modf(absv, &integral);
    degrees = (uint8_t)integral;

    double minutes_full = (absv - degrees) * 60.0;
    std::modf(minutes_full, &integral);
    minutes = (uint8_t)integral;

    double submin1_full = (minutes_full - minutes) * 100.0;
    std::modf(submin1_full, &integral);
    submin1 = (uint8_t)integral;

    double submin2_full = (submin1_full - submin1) * 100.0;
    std::modf(submin2_full, &integral);
    submin2 = (uint8_t)integral;
}
```

Decoded back to decimal degrees:

```
decimal = degrees + minutes/60 + submin1/6000 + submin2/600000
```

Direction is applied separately.

### 7.3 Altitude

* `altitude` is an unsigned 16-bit absolute value in meters.
* `altitude_ref` is `'P'` for positive altitude and `'M'` for negative altitude.  The camera rejects the payload if `altitude_ref` is not set correctly.

### 7.4 Example GPS payload construction

```c
struct nikon_geo_t {
    uint16_t header;             // 0x007F
    uint8_t  latitude_direction; // 'N' or 'S'
    uint8_t  latitude_degrees;
    uint8_t  latitude_minutes;
    uint8_t  latitude_submin1;
    uint8_t  latitude_submin2;
    uint8_t  longitude_direction; // 'E' or 'W'
    uint8_t  longitude_degrees;
    uint8_t  longitude_minutes;
    uint8_t  longitude_submin1;
    uint8_t  longitude_submin2;
    uint8_t  satellites;
    uint8_t  altitude_ref;       // 'P' or 'M'
    uint16_t altitude;           // absolute meters
    uint16_t year;
    uint8_t  month;
    uint8_t  day;
    uint8_t  hour;
    uint8_t  minute;
    uint8_t  second;
    uint8_t  subseconds;         // centiseconds
    uint8_t  valid;              // 0x01
...
Populate the fields, then write the 41-byte buffer to the `GEO` characteristic with a write-with-response.
...
## 9. Known Gaps and Important Notes
...
## Appendix: Standard Blowfish S-Box Constants

If you are implementing Blowfish from scratch rather than using an existing library, use these standard S-boxes (pi-derived, 4 × 256 entries).  They are the same constants used in the reference implementation and are required for the key schedule to produce the correct P-array/S-box values.
